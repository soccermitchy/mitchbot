...
function meta:hook(name,id,f,net)
    self.hooks[name] = self.hooks[name] or {}
    self.hooks[name][id] = {}
    self.hooks[name][id].f=f
    self.hooks[name][id].netname=net
    return id or f
end
...
function meta:invoke(name, ...)
	local hooks = self.hooks[name]
	if hooks then
		for id,h in pairs(hooks) do
			if h.f(self.netname,...) then
				return true
			end
		end
	end
...
function meta_preconnect:connect(_host, _port, _netname)
    ...
    netname = netname or error("network name required to connect",2)
	port = port or 6667
